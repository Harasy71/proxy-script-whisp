...
### SECProxyParser Status
- ✅ Base structure implemented and inherits from HTMLParser
- ✅ Section detection regex patterns defined (SEC_SECTION_PATTERNS)
- ✅ `parse_with_metadata()` method signature in place
- ✅ `_detect_section_header()` method working with pattern matching
- ✅ `_enrich_blocks()` adds SEC metadata (CIK, company_name, filing_date, document_type)
- ✅ Section header tracking propagates to subsequent blocks
- ✅ Table blocks extract `rows` (list[list[str]]) and `linearized_text` from metadata

### Test Requirements
All tests in `test_html_parser_sec.py` and `test_table_dual_format.py` require:
1. Parser returns `list[SECBlock]` instances
2. Each block populated with filing metadata from `FilingMetadata` object
3. Section header detection triggers on heading text matching patterns
4. Table blocks carry both structured rows and linearized text formats

### Key Implementation Details
- `_extract_table()` utility in `html_parser.py` already produces (rows, linearized) tuple
- HTMLParser base class populates `metadata["rows"]` for TABLE blocks
- SECProxyParser._enrich_blocks() reads metadata and constructs SECBlock instances
- Section detection is case-insensitive and normalizes to title case
